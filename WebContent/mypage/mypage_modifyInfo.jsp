...
<tr>
      ...
      <form id="send_mail" method="post" action="modifyinfo">
        ...
        <div id="idField">
          <label> 아이디</label>
          <input type="text" class="form-control" name="id" value ="${memDTO.mid }" readonly>
        </div>
		
		 <div id="nameField">
          <label> 이름 </label>
          <input type="text" class="form-control" name="name" value ="${memDTO.mname }" required>
        </div>
        
        
        <div id="emailField">
          <label> 이메일 </label>
          <input type="text" class="form-control" name="email" value ="${memDTO.memail }" required>
        </div>

        <div id="phoneField">
          <label> 비밀번호 </label>
          <input type="password" class="form-control" name="password" value="${memDTO.mpwd }" required>
        </div>
        
        <div id="genderField">
          <label> 성별 </label>
          <input type="radio" name="sex" value="male" <c:if test= '${memDTO.msex} == "male"'>checked</c:if>> Male
          <input type="radio" name="sex" value="female" <c:if test= '${memDTO.msex} == "female"'>checked</c:if>> Female
        </div>
        
        <div id="interestField">
          <label> 관심종목 </label>
				<input type="radio" name="interest" value="soccer" <c:if test= '${memDTO.minterest} == "soccer"'>checked</c:if>> Soccer
				<input type="radio" name="interest" value="basketball" <c:if test= '${memDTO.minterest} == "basketball"'>checked</c:if>> Basketball
        </div>
